...
if __name__ == '__main__':
    ...
    if not args.speed:
        ...
        # --- numpy (CPU) baseline -----------------------------------------------
        # There's no HIP-event equivalent on the CPU, so time.perf_counter() (a
        # monotonic wall clock) is the right tool here -- it's what everyone uses
        # to benchmark numpy/BLAS calls. Avoid time.process_time(): numpy's BLAS
        # is multithreaded, so process_time would sum CPU-seconds across all
        # threads instead of reporting actual elapsed time, inflating the result.
        A @ B.T  # warmup: lets BLAS spin up its thread pool / pick a kernel variant
        start = time.perf_counter()
        for _ in range(ITERS):
            A @ B.T
        numpy_s = (time.perf_counter() - start) / ITERS

        print(f"numpy (CPU) time: {numpy_s * 1e3:.3f} ms  ->  {FLOPS / numpy_s / 1e12:.3f} TFLOPS")
